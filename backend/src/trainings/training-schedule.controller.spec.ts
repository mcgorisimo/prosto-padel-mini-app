...
describe('TrainingScheduleController authenticated read boundary', () => {
  let app: NestFastifyApplication;
  let authenticate: jest.Mock;
  let scheduleRead: jest.Mock;
  const logs: unknown[][] = [];
  beforeEach(async () => {
    ...
    scheduleRead = jest.fn().mockResolvedValue({ outcome: 'not_configured', sessions: [] });
    const module = await Test.createTestingModule({
      ...
      providers: [SessionBearerGuard,
        ...
        { provide: TrainingScheduleService, useValue: { read: scheduleRead } },
      ],
    }).compile();
  ...
  it('returns the same server-owned schedule outcome for different authenticated owners', async () => {
