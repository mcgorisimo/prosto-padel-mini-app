...
export class PostgresCourtReservationRepository implements CourtReservationRepository {
  ...
  async create(transaction: PostgresTransaction, reservation: CourtReservation): Promise<CreateCourtReservationPersistenceResult> {
    try {
      const result = await transaction.query<ReservationRow>(`
        ...
        ) VALUES ($1,$2,$3,$4,$5,$6::timestamptz,$6,$7::timestamptz,$7,$8,$9,$10,$10,$10)
  ...
  async applyExactRefresh(transaction: PostgresTransaction, ownerAccountId: AccountId, reservationId: CourtReservationId, input: Readonly<{companyId:number;recordId:number;apiId:number;serviceId:number;courtId:number;startsAt:string;endsAt:string;deleted:boolean;now:number}>): Promise<ReservationRefreshPersistenceResult> {
    try {
      ...
      const updated=await transaction.query(`UPDATE backend_reservation.court_reservations SET status=$3,target_service_id=$4,target_resource_id=$5,target_datetime=$6::timestamptz,target_datetime_text=$6,target_end_datetime=$7::timestamptz,target_end_datetime_text=$7,version=version+1,updated_at=$8,status_changed_at=CASE WHEN status<>$3 THEN $8 ELSE status_changed_at END,terminal_at=CASE WHEN $3='cancelled' THEN $8 ELSE NULL END WHERE owner_account_id=$1 AND reservation_id=$2`,[ownerAccountId,reservationId,nextStatus,input.serviceId,input.courtId,input.startsAt,input.endsAt,input.now]);
      if(updated.rowCount!==1) throw new CourtReservationPersistenceError('transaction_conflict');
