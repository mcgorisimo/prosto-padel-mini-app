...
-- Read-only postcheck for public.join_match(p_match_id uuid).
-- Returns one jsonb column and reads PostgreSQL catalogs only.

with expected_function(function_name, identity_arguments) as (
  values ('join_match', 'p_match_id uuid')
),
function_catalog as (
  select
    p.oid,
...
function_checks as (
  select
    ...
    coalesce(f.pronargs = 1, false) as single_argument,
