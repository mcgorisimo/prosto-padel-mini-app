...
-- Read-only preflight for the private default backend rating state.
...
declare
  ...
  v_count bigint;
  v_accounts_oid oid :=
    pg_catalog.to_regclass('backend_auth.accounts')::oid;
  v_profiles_oid oid :=
...
begin
  ...
  if pg_catalog.has_database_privilege(
    ...
    raise exception 'PRECHECK_FAILED: backend_auth_app CREATE privileges are unsafe';
  end if;
  ...
  if pg_catalog.to_regclass('backend_auth.player_rating_states') is not null
    ...
    raise exception 'PRECHECK_FAILED: migration 019 relation or index name is already occupied';
  end if;
  ...
  if v_details_oid is null
    ...
    raise exception 'PRECHECK_FAILED: player_profile_details is not the canonical migration 018 relation';
  end if;

  if v_accounts_oid is null
     or v_profiles_oid is null
    ...
    raise exception 'PRECHECK_FAILED: accounts or player_profiles binding differs';
  end if;
  ...
  if (select pg_catalog.count(*)
     ...
     or not exists (
       ...
       where c.conrelid = v_profiles_oid
         ...
         and c.contype = 'f'
         and c.confrelid = v_accounts_oid
         and c.conkey = array[1]::smallint[]
         and c.confkey = array[
           (
             select a.attnum
             from pg_catalog.pg_attribute a
             where a.attrelid = v_accounts_oid
               and a.attname = 'id'
               and a.attnum > 0
               and not a.attisdropped
           )
  ...
  if v_count <> 15 then
    raise exception 'PRECHECK_FAILED: expected 15 backend_auth tables, found %',
      v_count;
  end if;
  ...
  if v_count <> 156 then
    raise exception 'PRECHECK_FAILED: expected 156 backend_auth constraints, found %',
      v_count;
  end if;
...
) as backend_auth_player_rating_state_precheck;
