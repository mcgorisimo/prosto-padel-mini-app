-- 041_backend_personal_data_processing_consent_ROLLBACK.sql
-- Safe only before any personal_data_processing evidence exists.
...
begin
  if v_acceptance_oid is null
    ...
    raise exception 'ROLLBACK_PRECONDITION_FAILED: consent ledger differs from migration 041';
  end if;
  ...
  if v_guard_oid is null
    ...
    raise exception 'ROLLBACK_PRECONDITION_FAILED: transition guard differs from migration 041';
  end if;
  ...
  if v_immutable_oid is null
    ...
    raise exception 'ROLLBACK_PRECONDITION_FAILED: immutable guard differs from migration 015';
  end if;
  ...
  if v_state_oid is null
    ...
    raise exception 'ROLLBACK_PRECONDITION_FAILED: current onboarding dependency differs';
  end if;
  ...
  if exists (
    ...
    raise exception 'ROLLBACK_BLOCKED: personal-data-processing evidence exists; use a forward migration';
  end if;
end;
$preconditions$;
...
begin
  if exists (
    ...
    raise exception 'ROLLBACK_BLOCKED: personal-data-processing evidence appeared while locking; use a forward migration';
  end if;
end;
...
begin
  if pg_catalog.obj_description(v_acceptance_oid, 'pg_class') is distinct from
    ...
    raise exception 'ROLLBACK_ASSERTION_FAILED: consent ledger was not restored';
  end if;
  ...
  if pg_catalog.obj_description(v_guard_oid, 'pg_proc') is distinct from
    ...
    raise exception 'ROLLBACK_ASSERTION_FAILED: transition guard was not restored';
  end if;

  if pg_catalog.obj_description(v_immutable_oid, 'pg_proc') is distinct from
       '015_backend_auth_foundation:'
         || pg_catalog.md5(
           pg_catalog.pg_get_functiondef(v_immutable_oid)
         ) then
    raise exception 'ROLLBACK_ASSERTION_FAILED: immutable guard changed';
  end if;
  ...
  if pg_catalog.obj_description(v_state_oid, 'pg_class') is distinct from
    ...
    raise exception 'ROLLBACK_ASSERTION_FAILED: unrelated onboarding relation changed';
  end if;
  ...
  if exists (
    ...
    raise exception 'ROLLBACK_ASSERTION_FAILED: new consent evidence remains';
  end if;
  ...
  if (
    ...
    raise exception 'ROLLBACK_ASSERTION_FAILED: three-kind constraint was not restored';
  end if;
  ...
  if (
    ...
    raise exception 'ROLLBACK_ASSERTION_FAILED: consent immutability trigger changed';
  end if;
  ...
  if not pg_catalog.has_function_privilege(
    ...
    raise exception 'ROLLBACK_ASSERTION_FAILED: post-036 function ACL was not restored';
  end if;

  v_guard_definition := pg_catalog.pg_get_functiondef(v_guard_oid);
  if pg_catalog.strpos(v_guard_definition, 'v_consent_kind_count bigint') = 0
     or pg_catalog.strpos(v_guard_definition, 'v_consent_kinds text[]') <> 0
     or pg_catalog.strpos(v_guard_definition, 'personal_data_processing') <> 0 then
    raise exception 'ROLLBACK_ASSERTION_FAILED: migration-037 guard definition differs';
  end if;
end;
$assertions$;
...
select
  '041_backend_personal_data_processing_consent rolled back before new evidence use'
  as result;
