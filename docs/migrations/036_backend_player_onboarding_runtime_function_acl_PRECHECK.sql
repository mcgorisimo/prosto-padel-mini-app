-- 036_backend_player_onboarding_runtime_function_acl_PRECHECK.sql
-- Read-only verification of the exact migration-035 ACL baseline.
...
begin
  ...
  loop
    ...
    if v_function_oid is null
      ...
      raise exception 'PRECHECK_FAILED: function fingerprint differs for %',
    ...
    if pg_catalog.has_function_privilege(
      ...
      raise exception 'PRECHECK_FAILED: expected migration-035 function prohibition differs for %',
        v_expected.signature;
    end if;
  end loop;
end;
...
select pg_catalog.jsonb_build_object(
  ...
  'ready', true,
  'source_migration', '035_backend_player_onboarding_foundation',
