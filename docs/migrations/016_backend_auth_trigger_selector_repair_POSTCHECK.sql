...
-- Read-only verification of the canonical repaired function bodies and unchanged attachments.
...
begin
  ...
  if not exists (
    ...
    where n.nspname = 'backend_auth'
      and pg_catalog.pg_get_userbyid(n.nspowner) = 'backend_auth_owner'
  ) then
    raise exception 'POSTCHECK_FAILED: schema backend_auth is missing or has an unexpected owner';
  end if;
  ...
  loop
    if not exists (
      ...
      raise exception 'POSTCHECK_FAILED: required table backend_auth.% is missing or has an unexpected kind',
  ...
  if v_count <> 4 then
    raise exception 'POSTCHECK_FAILED: target functions are missing or have unexpected overloads';
  end if;
  ...
  for v_expected in
    ...
    from (values
      (
        'assert_player_profile_consistency',
        'c8966f77d1414d2b1f9b301d91801677',
        'v_account_id:=case',
        'iftg_table_name=''accounts''theniftg_op=''delete''thenv_account_id:=old.id;elsev_account_id:=new.id;endif;elsiftg_table_name=''player_profiles''theniftg_op=''delete''thenv_account_id:=old.account_id;elsev_account_id:=new.account_id;endif;'
      ),
      (
        'assert_external_identity_aliases',
        'ff511fdff249a6585980182ea288a682',
        'v_identity_id:=case',
        'iftg_table_name=''external_identities''theniftg_op=''delete''thenv_identity_id:=old.id;elsev_identity_id:=new.id;endif;elsiftg_table_name=''external_identity_lookup_digests''theniftg_op=''delete''thenv_identity_id:=old.identity_id;elsev_identity_id:=new.identity_id;endif;'
      ),
      (
        'assert_session_consistency',
        '1ca046173bb7b8fbd35d55adf4c1d64b',
        'v_family_id:=case',
        'iftg_table_name=''auth_session_families''theniftg_op=''delete''thenv_family_id:=old.id;elsev_family_id:=new.id;endif;elsiftg_table_name=''auth_session_credentials''theniftg_op=''delete''thenv_family_id:=old.family_id;elsev_family_id:=new.family_id;endif;elsiftg_table_name=''auth_session_commands''theniftg_op=''delete''thenv_family_id:=old.family_id;elsev_family_id:=new.family_id;endif;'
      ),
      (
        'assert_otp_consistency',
        '2275cb792d3a4fee9aaeee950d5dd376',
        'v_challenge_id:=case',
        'iftg_table_name=''otp_challenges''theniftg_op=''delete''thenv_challenge_id:=old.id;elsev_challenge_id:=new.id;endif;elsiftg_table_name=''otp_commands''theniftg_op=''delete''thenv_challenge_id:=old.challenge_id;elsev_challenge_id:=new.challenge_id;endif;'
      )
    ) expected(function_name, normalized_source_md5, unsafe_selector, safe_selector)
  loop
    select
      p.oid,
    ...
    into v_function_oid, v_normalized_source, v_comment
    from pg_catalog.pg_proc p
    join pg_catalog.pg_namespace n on n.oid = p.pronamespace
    ...
    if not found then
      raise exception 'POSTCHECK_FAILED: canonical attributes differ for function %',
        v_expected.function_name;
    end if;

    if pg_catalog.md5(v_normalized_source)
       <> v_expected.normalized_source_md5 then
      raise exception 'POSTCHECK_FAILED: canonical body differs for function %',
        v_expected.function_name;
    end if;

    if pg_catalog.strpos(v_normalized_source, v_expected.unsafe_selector) > 0 then
      raise exception 'POSTCHECK_FAILED: unsafe shared CASE selector remains in function %',
        v_expected.function_name;
    end if;

    if pg_catalog.strpos(v_normalized_source, v_expected.safe_selector) = 0 then
      raise exception 'POSTCHECK_FAILED: safe IF/ELSIF selector is missing in function %',
        v_expected.function_name;
    end if;

    if v_comment is distinct from (
      '016_backend_auth_trigger_selector_repair:'
        || pg_catalog.md5(pg_catalog.pg_get_functiondef(v_function_oid))
    ) then
      raise exception 'POSTCHECK_FAILED: migration 016 comment fingerprint differs for function %',
        v_expected.function_name;
    end if;
    ...
    if exists (
      ...
      raise exception 'POSTCHECK_FAILED: function ACL exceeds the migration 015 boundary for %',
        v_expected.function_name;
    end if;
  end loop;
  ...
  loop
    ...
    if v_count <> 1 then
      raise exception 'POSTCHECK_FAILED: trigger attachment % differs',
  ...
  if v_count <> 9 then
    raise exception 'POSTCHECK_FAILED: expected exactly 9 target trigger attachments, found %',
      v_count;
  end if;
  ...
  if v_count <> 14 then
    raise exception 'POSTCHECK_FAILED: expected 14 backend_auth tables, found %',
      v_count;
  end if;
  ...
  where n.nspname = 'backend_auth';
  if v_count <> 146 then
    raise exception 'POSTCHECK_FAILED: expected 146 backend_auth constraints, found %',
      v_count;
  end if;
  ...
  if v_count <> 131 then
    raise exception 'POSTCHECK_FAILED: expected 131 backend_auth table constraints, found %',
      v_count;
  end if;
...
) as backend_auth_trigger_selector_repair_postcheck;
