...
### 2026-08-28 — local Telegram bot-notification vertical slice

- From exact clean `main` base `b489d5acef2a6f84044be39586e3b4842c3449b5`, added a disabled-by-default transactional per-recipient Telegram intent dispatcher, safe event-specific Mini App links, master-plus-category preferences, durable 24h/2h reminders, and bounded 5-minute exact-record YCLIENTS read reconciliation (7-day horizon, batch/request budget 10, concurrency 1, lease, no list/write fallback). Migration 043 is `unapplied_local_artifact`; no external call, DB/migration apply, secret/env mutation, push, merge or deployment occurred.
- Runtime producers are wired for invitation, FIFO waitlist promotion, participant join/leave to organizer, chat to current audience excluding author, reminders, initial reservation confirmation, and exact YCLIENTS reschedule/deleted-record cancellation; YCLIENTS 404/unknown remains stale fail-closed. `match_cancelled` has contract/renderer only because no canonical backend cancellation producer exists. Payment notification is an unwired runtime-disabled D4 port; payment fields were untouched. Telegram ambiguity is terminal `delivery_unknown` with no blind resend, so the honest guarantee is one durable intent plus at-most-once under ambiguous delivery, not impossible strict Bot API exactly-once.
- Final gates: focused backend `95/95` plus migration recheck `3/3`, focused frontend `5/5`, backend unit `4025/4025`, typecheck, backend E2E `4/4`, backend build, root E2E `111/111` with `1` intentional skip, root build (existing chunk advisory only), runtime-compose static check and `git diff --check` passed. Independent final exact-diff review is `CLEAR`, `P0=0`, `P1=0`, with only the two disclosed blockers above.
- Local checkpoint is this commit on `codex/telegram-notifications-vertical-slice`; no push. Because backend, frontend bundle, compose/config and an unapplied migration artifact change, status is `deployment_deferred_by_user`. The latest documented live test handoff remains Selectel frontend `14d8e74eca222974bd8c4ba6bfc984c76efc0f37` and backend `f55ba9318c92b97c6fa5e0b0defcdd752fab56cb`; a separately authorized REG.RU test rollout must first reconcile the target, integrate exact `main`, precheck/apply/postcheck 043, deploy exact images with both workers initially disabled, then pass health/business-smoke/log gates before staged enablement.

### 2026-08-29 — D5.N exact invitation canary gate
...
### 2026-09-07 — Remove only the Home events heading decoration

- Clean starting origin/main and verified Selectel TEST frontend:
  04efbb8c9f8d28412f9fa59a0cf169df2a0fc33b. Previous rollout is healthy:
  frontend5632971f, backendd1dd066, ten running/nine healthy/zero restarts.
- Owner requested removal only of the standalone decorative racket/ball to
  the right of My events heading. Home.jsx removes that three-line wrapper;
  training filter, approved 32px group CTA, event icons and Swords unchanged.
  Focused test checks no SVG in heading row, preserved filter/badge/CTA icons,
  32px CTA and existing click behavior. Text/layout classes and logic unchanged.
- Root unit168/168 PASS; E2E --workers=4:122 PASS+1 existing intentional skip
  on first run; build PASS(1628 modules); changed-file ESLint/diff-check PASS.
  Fresh portrait375x667 and landscape667x375 screenshots generated by real
