...
- Версия: `owner_input_checklist_v0.1`.
- Дата: `2026-08-23`.
...
- Реквизиты нового ООО ожидаются: `2026-08-26`.
...
- `pending_26_aug` — ожидается вместе с регистрационными данными ООО;
...
|---|---|---|---|---|
| `company.full_name` | Полное имя точно по ЕГРЮЛ | `pending_26_aug` | `{{LEGAL_ENTITY_FULL_NAME}}` | Все документы, оператор ПД, продавец |
| `company.short_name` | Сокращённое имя точно по ЕГРЮЛ | `pending_26_aug` | `{{LEGAL_ENTITY_SHORT_NAME}}` | Заголовки и обращения |
| `company.inn` | ИНН | `pending_26_aug` | `{{INN}}` | Все документы и чеки |
| `company.kpp` | КПП | `pending_26_aug` | `{{KPP}}` | Реквизиты компании |
| `company.ogrn` | ОГРН | `pending_26_aug` | `{{OGRN}}` | Все документы |
| `company.legal_address` | Юридический адрес | `pending_26_aug` | `{{LEGAL_ADDRESS}}` | Оператор, продавец, обращения |
| `company.postal_address` | Почтовый адрес либо отметка «совпадает с юридическим» | `pending_26_aug` | `{{POSTAL_ADDRESS}}` | Претензии и privacy requests |
| `club.official_address` | Полный официальный адрес клуба | `pending_26_aug` | `{{CLUB_OFFICIAL_ADDRESS}}` | Место оказания услуг |
