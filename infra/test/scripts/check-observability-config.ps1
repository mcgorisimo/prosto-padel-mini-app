...
$null = Read-RequiredFile 'infra/test/observability/grafana/provisioning/alerting/.gitkeep'
$null = Read-RequiredFile 'infra/test/observability/grafana/provisioning/plugins/.gitkeep'
...
Assert-Contains $compose '(?ms)^  grafana:.*?networks:\s*\r?\n\s+- observability_internal\s*\r?\n\s+- observability_access' 'OBSERVABILITY_GRAFANA_ACCESS_NETWORK_MISSING'
